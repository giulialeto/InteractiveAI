...
then
    ...
    curl -X DELETE $url/perimeters/$1 -H "Authorization:Bearer $token"
    echo ""
    echo "Create perimeter $1 on $url"
    curl -X POST $url/perimeters -H "Content-type:application/json" -H "Authorization:Bearer $token" --data @$1.json
    echo ""
fi
